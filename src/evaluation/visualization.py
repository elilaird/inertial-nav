...
def plot_trajectory_2d(p_pred, p_gt, seq_name="", aligned=False):
    """
    ...
    ax.plot(p_pred[:, 0], p_pred[:, 1], 'r-', label='Predicted', linewidth=1.5)
    ax.plot(p_gt[0, 0], p_gt[0, 1], 'go', markersize=8, label='Start')
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
...
def plot_trajectory_3d(p_pred, p_gt, seq_name=""):
    ...
    ax.plot(p_gt[:, 0], p_gt[:, 1], p_gt[:, 2], 'b-', label='Ground Truth', linewidth=1.5)
    ax.plot(p_pred[:, 0], p_pred[:, 1], p_pred[:, 2], 'r-', label='Predicted', linewidth=1.5)
    ax.set_xlabel('x (m)')
    ax.set_ylabel('y (m)')
    ax.set_zlabel('z (m)')
...
def plot_error_timeline(errors, timestamps=None, seq_name="",
...
def plot_covariance_timeline(measurements_covs, timestamps=None, seq_name=""):
    ...
    title = f"Measurement Covariances - {seq_name}" if seq_name else "Measurement Covariances"
    ax.set_title(title)
    ax.legend()
    ax.grid(True)
    fig.tight_layout()
...
def plot_state_estimates(p_pred, p_gt, v_pred=None, v_gt=None,
                         timestamps=None, seq_name=""):
    """
    ...
    for i in range(3):
        axs[0].plot(timestamps, p_gt[:, i], colors_gt[i], label=f'{labels[i]} GT')
        axs[0].plot(timestamps, p_pred[:, i], colors_pred[i], linestyle='--',
    ...
    if v_pred is not None and v_gt is not None:
        for i in range(3):
            axs[1].plot(timestamps, v_gt[:, i], colors_gt[i], label=f'{labels[i]} GT')
            axs[1].plot(timestamps, v_pred[:, i], colors_pred[i], linestyle='--',
                        label=f'{labels[i]} pred')
